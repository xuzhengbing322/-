...
let { skuNum } = toRefs(data);

// 请求数据
store.getDetailData(route.params.skuid);

let { categoryView, skuInfo, spuSaleAttrList, skuImageList } =
  storeToRefs(store);
...
//skuid：商品，skuNum：购买数量。将购买的商品和数量发送给服务器存储，然后传参跳转到添加成功页面
// await getShopCarData返回的promise的成功和失败。成功进行路由跳转，失败就提示信息。
async function goShopCar() {
  try {
    await store.getShopCarData(route.params.skuid, data.skuNum);
    // 路由跳转
    router.push({
      name: "addcartsuccess",
      query: { skuNum: data.skuNum },
    });
    // 路由传参：商品的skuInfo对象中的数据。JSON.stringify()将对象转变为字符串
